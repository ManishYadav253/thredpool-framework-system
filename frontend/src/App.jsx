...
import { Activity, Server, Users, Zap, Power, RotateCcw, List, BarChart2, Settings } from 'lucide-react';
...
function App() {
  ...
  const exportLogs = () => {
    ...
    downloadAnchorNode.setAttribute("download", "threadpool_logs.json");
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-6 font-sans">
      <header className="max-w-6xl mx-auto mb-8 flex justify-between items-center bg-slate-900/50 p-6 rounded-2xl border border-slate-800 backdrop-blur-sm shadow-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">High-performance concurrency visualizer</p>
          </div>
        </div>
        <div className="flex gap-3">
          <button onClick={() => setActiveTab('dashboard')} className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'dashboard' ? 'bg-blue-600 text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>Dashboard</button>
          <button onClick={() => setActiveTab('history')} className={`px-4 py-2 rounded-lg font-medium transition-colors ${activeTab === 'history' ? 'bg-blue-600 text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>History</button>
          <button onClick={() => setActiveTab('compare')} className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${activeTab === 'compare' ? 'bg-purple-600 text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#0c111c] border border-slate-800 p-4 rounded-2xl shadow-lg h-[350px] flex flex-col font-mono text-sm">
              <div className="flex justify-between items-center mb-3 border-b border-slate-800 pb-2">
                <span className="text-slate-400 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div> System Logs</span>
              </div>
              <div className="flex-1 overflow-y-auto space-y-1">
                {status.logs.map(log => (
                  <div key={log.id} className="text-slate-300">
                    <span className="text-slate-500">[{log.time}]</span> <span className={log.message.includes('completed') ? 'text-emerald-400' : log.message.includes('started') ? 'text-blue-400' : 'text-slate-300'}>{log.message}</span>
                  </div>
                ))}
